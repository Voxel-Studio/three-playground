...
function Scene({ p = new THREE.Vector3() }) {
  ...
  useEffect(() => {
    setIos(
      !/android|windows|linux/.test(window.navigator.userAgent.toLowerCase())
    );
  ...
  useFrame((state, dt) => {
    ...
    } else if (state.camera.position.z >= 10 && state.camera.position.z < 15) {
      cameraTargetRotation.y =
        (Math.PI / 4) * ((state.camera.position.z - 10) / (15 - 10));
    } else if (state.camera.position.z >= 15 && state.camera.position.z < 20) {
      cameraTargetRotation.y =
        (Math.PI / 4) * (1 - (state.camera.position.z - 15) / (20 - 15));
    } else if (state.camera.position.z >= 20 && state.camera.position.z < 29) {
      cameraTargetRotation.y =
        ((-Math.PI / 4) * (state.camera.position.z - 20)) / (29 - 20);
    } else if (state.camera.position.z >= 29 && state.camera.position.z < 40) {
      cameraTargetRotation.y =
        (-Math.PI / 4) * (1 - (state.camera.position.z - 29) / (40 - 29));
    } else {
      cameraTargetRotation.y = 0;
    }
  ...
  return (
    <group>
      ...
      <Image
        ...
        scale={[imageScale(1.972 / 2), imageScale(2.86 / 2), imageScale(1)]}
        transparent
      />
      {/* Top dots */}
      <Image
      ...
      <Image
        ...
        rotation={[0, 0, 0]}
        scale={[imageScale(0.045 / 1), imageScale(1.85 / 1), imageScale(1)]}
        transparent
      />
      {/* Yellow one */}
      <Frame
      ...
      <Image
        ...
        rotation={[0, 0, 0]}
        scale={[imageScale(1.972 / 2), imageScale(4.34 / 2), imageScale(1)]}
        transparent
      />
      {/* Purple one */}
      <Frame
        ...
        r={[0, Math.PI / 2, 0]}
        s={[imageScale(2.5), imageScale(GOLDEN_RATIO), imageScale(0.075)]}
        url="/tiktok-1.jpeg"
      />
      {/* Transforming spaces */}
      <Image
        ...
        rotation={[0, Math.PI / 2, 0]}
        scale={[imageScale(6.19 / 4), imageScale(2.7 / 4), imageScale(1)]}
        transparent
      />
      {/* Pole */}
      <Image
      ...
      <Image
        ...
        rotation={[0, -Math.PI / 2, 0]}
        scale={[imageScale(5.04 / 4), imageScale(2.51 / 4), imageScale(1)]}
        transparent
      />
      {/* Teal one */}
      <Frame
        ...
        r={[0, -Math.PI / 2, 0]}
        s={[imageScale(1.8), imageScale(GOLDEN_RATIO), imageScale(0.075)]}
      ...
      <Image
        ...
        position={[0, 1, 38]}
        rotation={[0, 0, 0]}
        scale={[imageScale(7 / 2), imageScale(1.517 / 2), imageScale(1)]}
        transparent
        name="noFade"
      />
    </group>
  ...
  return (
    <group position={p} rotation={r}>
      ...
      >
        ...
        <Image url={url} position={[0, 0, 0.7]} scale={[0.975, 0.975, 1]} />
      </mesh>
    </group>
  );
}
