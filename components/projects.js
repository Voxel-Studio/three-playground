...
const init = (setSelected) => {
  ...
  for (let i = 0; i < totalPoints; i++) {
    ...
    loader.load("/lexend-deca.json", function (font) {
      ...
      text.position.set(
        ...
        desktop
          ? radius * 1.8 * Math.cos(angle)
          : radius * 1.7 * Math.cos(angle),
  ...
  const render = () => {
    ...
    if (rot > theta * 7 + theta * 0.5 || rot <= theta * 0.5) {
      ...
      setSelected(desktop ? 7 : 1);
      selected = desktop ? 7 : 1;
    } else if (rot > theta + theta * 0.5 && rot <= theta * 2 + theta * 0.5) {
      setSelected(desktop ? 6 : 2);
      selected = desktop ? 6 : 2;
    } else if (
      rot > theta * 2 + theta * 0.5 &&
      rot <= theta * 3 + theta * 0.5
    ) {
      setSelected(desktop ? 5 : 3);
      selected = desktop ? 5 : 3;
    } else if (
      rot > theta * 3 + theta * 0.5 &&
      rot <= theta * 4 + theta * 0.5
    ) {
      setSelected(desktop ? 4 : 4);
      selected = desktop ? 4 : 4;
    } else if (
      rot > theta * 4 + theta * 0.5 &&
      rot <= theta * 5 + theta * 0.5
    ) {
      setSelected(desktop ? 3 : 5);
      selected = desktop ? 3 : 5;
    } else if (
      rot > theta * 5 + theta * 0.5 &&
      rot <= theta * 6 + theta * 0.5
    ) {
      setSelected(desktop ? 2 : 6);
      selected = desktop ? 2 : 6;
    } else if (
      rot > theta * 6 + theta * 0.5 &&
      rot <= theta * 7 + theta * 0.5
    ) {
      setSelected(desktop ? 1 : 7);
      selected = desktop ? 1 : 7;
    }
    ...
    if (brackets)
      brackets.style.transform = desktop
