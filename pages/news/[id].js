...
export const getStaticProps = async (context) => {
  ...
  switch (id) {
    case "lorem-ipsum-1":
      data = newsItems[0];
      itemNext = newsItems[1];
      itemPrev = newsItems[2];
      break;
    case "onsite-at-excel-london":
      data = newsItems[1];
      itemNext = newsItems[2];
      itemPrev = newsItems[0];
      break;
    case "lorem-ipsum-2":
      data = newsItems[2];
      itemNext = newsItems[0];
      itemPrev = newsItems[1];
      break;
    default:
      break;
  }
  ...
  return {
    props: { item: data, itemPrev, itemNext },
  };
};

export default function Article({ item, itemPrev, itemNext }) {
  useEffect(() => {
    ...
    <div className={titleStyles.container}>
      ...
      <div className={styles.newsItemWrapper}>
        <p className="section">
          ...
          ultricies imperdiet, tellus libero malesuada risus, non commodo lectus
          eros vel risus. Interdum et malesuada fames ac ante ipsum primis in
          faucibus. Suspendisse sit amet bibendum lectus, blandit ultrices nunc.
          Nunc laoreet purus lacus, vitae pellentesque enim semper vitae.
        </p>
        <p className="section">
          Nulla feugiat sit amet nunc ac pulvinar. Etiam sapien dui, tempor
          porttitor sollicitudin eget, commodo sed nisi. Praesent urna nisl,
          convallis aliquam tempor quis, porta condimentum mauris. Aenean
          scelerisque eros magna, nec venenatis odio sodales et. Sed sodales
          turpis turpis, quis varius lectus posuere porttitor. Duis ac tincidunt
          nibh, vel porttitor neque. Duis magna erat, convallis a massa ut,
      ...
      <div className="moreNews">
        <div
          className="newsCard"
          onClick={() => {
            router.push(`/news/${itemPrev.id}`);
          }}
        >
          <img src={itemPrev.img} alt="" />
        ...
        <div
          className="newsCard"
          onClick={() => {
            router.push(`/news/${itemNext.id}`);
          }}
        >
          <p>{itemNext.title}</p>
          <p className="nextArticle">NEXT</p>
          <div className="nextLine" />
          <img src={itemNext.img} alt="" />
        </div>
      </div>
      <div className="wrapper">
        <div
